...
class _EditReuniaoContentState extends State<EditReuniaoContent> {
    ...
    _carregarClientes().then((_) {
      Future.delayed(Duration.zero, () {
        ...
        if (novaReuniao == true) {
          setState(() {
            _isLoading = false;
            reuniao = Reuniao(id: '', assunto: '', dataHora: DateTime.now(), horaFim: TimeOfDay.now());
  ...
  void _carregarDadosReuniao() {
    if (reuniao != null) {
      ...
      _dataHoraController.text = DateFormat('dd/MM/yyyy HH:mm').format(reuniao!.dataHora);
    ...
    if (selectedDate != null) {
      ...
      if (selectedTime != null) {
        setState(() {
          ...
          _dataHoraController.text = DateFormat('dd/MM/yyyy HH:mm').format(_selectedDateTime!);
        });
      }
    }
  }
  ...
  Future<void> _salvarReuniao() async {
    if (novaReuniao) {
      ...
      await databaseService.adicionarReuniao(_selectedCliente!.id!, novaReuniao);
  ...
  Future<String> _obterProximoIdReuniao() async {
    List<Reuniao> reunioes = await databaseService.listarReunioes(_selectedCliente!.id!);
    if (reunioes.isEmpty) {
      return '1';
    }

    List<int> ids = reunioes.map((reuniao) => int.tryParse(reuniao.id!) ?? 0).toList();
